...
NGame = {
	...
	LAG_DAYS_FOR_LOWER_SPEED = 200,					-- Days of client lag for decrease of gamespeed
...
NDiplomacy = {
	...
	TRUCE_PERIOD_AFTER_KICKING_FROM_FACTION = 0,				-- Truce period after kicking someone from faction in days.
	...
	BASE_STAGE_COUP_DAILY_PP = 999,					-- Daily pp cost for staging a coup
	BASE_STAGE_COUP_TOTAL_COST = 0, 				-- Equipment consume factor for stage coup.
	...
	VOLUNTEERS_PER_COUNTRY_ARMY = 1.0,				-- Each army unit owned by the source country contributes this amount of volunteers to the limit.
	...
	TENSION_NO_CB_WAR = 12,							-- Amount of tension generated by a no-CB wargoal
	...
	BASE_CONDITIONAL_PEACE_WARESCORE_RATIO = 0.0, 	-- Warscore ratio needed for the losing side to able to surrender.
	BASE_CONDITIONAL_PEACE_MONTHS = 1,				-- War length must be before a surrender i possible.
	...
	PEACE_SCORE_PER_PASS = 1.0,						-- When you pass in a peace deal you gain this percent of your original score
	...
	LL_TO_PUPPET_AUTONOMY_DAILY_FACTOR = 0.0,		-- If overlord lend leases equipment to puppet of higher tech level as they have, puppet losses autonomy
	...
	LICENSE_ACCEPTANCE_OPINION_FACTOR = 0,		-- Opinion modifier for acceptance of license production requests.
	LICENSE_ACCEPTANCE_PUPPET_BASE = 0,			-- Acceptance modifier for puppets requesting production licenses.
	LICENSE_ACCEPTANCE_TECH_DIFFERENCE = -5, 		-- Acceptance modifier for each year of technology difference.
	LICENSE_ACCEPTANCE_TECH_DIFFERENCE_BASE = 0,    -- Acceptance base for tech difference
	LICENSE_ACCEPTANCE_SAME_FACTION = 0,			-- Acceptance modifier for being in the same faction
...
NCountry = {
	...
	RESISTANCE_STRENGTH_FROM_UNIT = 0.0,			-- How much strength ticking speed is reduced from each unit in state (multiplies subunit value resistance_suppress)
	RESISTANCE_STRENGTH_FROM_NEIGHBORS = 0.0, 		-- Multiplies how much resistance can spread from one state to its neighbors, a state will spread whatever is highest of its victorypoints resistance increase or half of any of its neighbors spread, multiplied by this
	RESISTANCE_DECAY_WHEN_NO_GROWTH = 0.100,		-- Resistance will fall by this much each day if there is nothing increasing it ( no VPs and no spread from neighbors )
	...
	REINFORCEMENT_EQUIPMENT_DELIVERY_SPEED = 0.6,	-- Modifier for army equipment reinforcement speed
	REINFORCEMENT_MANPOWER_DELIVERY_SPEED = 25.0,	-- Modifier for army manpower reinforcement delivery speed (travel time)
	...
	EQUIPMENT_UPGRADE_CHUNK_MAX_SIZE = 1000,		-- Maximum chunk size of equipment upgrade distribution per update.
	COUNTRY_SCORE_MULTIPLIER = 1.0,					-- Weight of the country score.
	ARMY_SCORE_MULTIPLIER = 0.0,					-- Based on number of armies.
	NAVY_SCORE_MULTIPLIER = 0.0,					-- Based on number of navies.
	AIR_SCORE_MULTIPLIER = 0.0,						-- Based on number of planes (which is typically a lot).
	INDUSTRY_SCORE_MULTIPLIER = 0.0,				-- Based on number of factories.
	PROVINCE_SCORE_MULTIPLIER = 0.0,				-- Based on number of controlled provinces.
	...
	MAX_INTELLIGENCE_AIR_DATA_DEVIATION = 0.5,		-- Max deviation in estimating enemy air planes amount ( 0.0 - 1.0 )
	...
	MIN_MANPOWER_RATIO = 0.25,						-- Min manpower ratio to show manpower alert
	...
	RESISTANCE_IMPORTANT_LEVEL = 0.50,				-- Level when resistance becomes dangerous
	...
	NUM_DAYS_TO_FULLY_DELETE_STOCKPILED_EQUIPMENT = 9999,	 -- time in days to fully delete equipments from stockpile. when you delete an equipment, they go to a temporary hidden pool which still can be seized 
	...
	BASE_FUEL_GAIN = 8,							-- base amount of fuel gained hourly, independent of excess oil
	BASE_FUEL_CAPACITY = 200000,						-- base amount of fuel capacity
	...
	GIE_DIVISION_ATTACK_BONUS_ON_CORE = 0.0, -- Attack bonus factor when fighting on cores.
	GIE_DIVISION_DEFENSE_BONUS_ON_CORE = 0.0, -- Defense bonus factor when fighting on cores.
...
NProduction = {
	...
	LICENSE_EQUIPMENT_TECH_SPEED_PER_YEAR = -0.00,		-- MIC speed modifier for licensed equipment for each year of difference between actual and latest equipment
...
NPolitics = {
	...
	LEADER_TRAITS_XP_SHOW = 1.00,			-- Amount of XP a trait needs to be shown in tooltips of a leader.
	...
	DEFAULT_OCCUPATION_POLICY = 3,		-- Defaullt value for occupation policy
...
NBuildings = {
	...
	DESTRUCTION_COOLDOWN_IN_WAR = 150,	-- Number of days cooldown between removal of buildings in war times
...
NMilitary = {
	...
	CORPS_COMMANDER_DIVISIONS_CAP = 100,			-- how many divisions a corps commander is limited to. 0 = inf, < 0 = blocked
	...
	RECON_SKILL_IMPACT = 7, -- how many skillpoints is a recon advantage worth when picking a tactic.
	...
	RIVER_CROSSING_PENALTY = -0.10,                 -- small river crossing
	RIVER_CROSSING_PENALTY_LARGE = -0.20,           -- large river crossing
	...
	SHIP_MORALE_TO_ORG_REGAIN_BASE = 1.0,			   -- Base org regain per hour
	BASE_NIGHT_ATTACK_PENALTY = -0.5,
	EXILE_EQUIPMENT = 1.0,						   -- Amount of equipment to keep
	EXILE_ORG = 0.0,							   -- Amount of org to keep
	EXPERIENCE_LOSS_FACTOR = 0.01,                 -- percentage of experienced solders who die when manpower is removed
	...
	MAX_RELATIVE_COMBAT_DAMAGE_TO_MODIFY_XP = 0,			-- you gain more XP if you are doing more damage relative to enemy, this is the max relative amount to gain following RATe
	XP_GAIN_FACTOR_FOR_MAX_RELATIVE_COMBAT_DAMAGE = 0.0,	-- XP factor scaling for max relative combat damage
	
	XP_DECAY_RATE_PER_HOUR_IN_COMBAT = 0.0,				-- you get reduced XP as combat drags
	...
	ARMY_FUEL_COST_MULT = 0.46,						-- fuel cost multiplier for all army related stuff
	...
	FUEL_CAPACITY_DEFAULT_HOURS = 200              -- default capacity if not specified
},
...
NAir = {
	...
	EFFICIENCY_REGION_CHANGE_DAILY_GAIN_STRATEGIC_BOMBER = 0.018,	-- How much efficiency to regain per day. Gain applied hourly.
	...
	FUEL_COST_MULT = 0.22, -- fuel multiplier for all air missions
...
NNavy = {
	...
	CARRIER_ONLY_COMBAT_ACTIVATE_TIME = 4,							-- hours from start of combat when carriers get to fight
	...
	FUEL_COST_MULT = 0.07, -- fuel multiplier for all naval missions
...
NAI = {
	...
	DIPLOMACY_FACTION_TAKE_OVER_RELUCTANCE_VERSUS_HUMAN = 5.0,	-- Multiplier penalty for how much stronger than a human faction member an AI country must be to choose to assume faction leadership.
	...
	DIPLOMACY_REJECTED_WAIT_MONTHS_BASE = 8,	-- AI will not repeat offers until at least this time has passed, and at most the double
	...
	MAX_VOLUNTEER_ARMY_FRACTION = 0.00001,			-- Countries will not send more than their forces time this number to aid another country
	...
	DIVISION_UPGRADE_MIN_XP = 400,				-- Minimum XP before attempting to upgrade a division template.
	...
	AT_WAR_THREAT_FACTOR = 4.0,					-- How much increase in threat does AI feel for being in war against osmeone
